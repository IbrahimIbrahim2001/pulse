...
import type { StoryType } from "../../types/chat";
            ...
            <div className="w-full sticky top-0 left-0 bg-background z-50">
                ...
                <div className="flex items-center px-4 mt-4 h-16 rounded-sm hover:bg-muted/50 transition-colors duration-200 border-b border-border/50 group">
                    ...
                    <div className="flex flex-col items-center justify-between ml-4">
                        <p className="font-semibold text-foreground w-full group-hover:text-primary group-hover:animate-pulse transition-colors duration-200 truncate">
                            My Status
                        </p>
...
function Stories({ myStories }: { myStories: StoryType[] }) {
    const text = myStories.length > 1 ? "stories" : "story";
    const firstStoryId = myStories[0]?.id;
    return (
        <Link href={{ pathname: `/status/${firstStoryId}` }} className="w-full">
            <p className="text-sm text-muted-foreground font-semibold">
                {myStories.length} {text}
            </p>
        </Link>
    );
}